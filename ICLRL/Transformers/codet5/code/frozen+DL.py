...
class CodeT5FeatureExtractor(nn.Module):
    """Frozen CodeT5 feature extractor with enhanced classifier head"""
    
    def __init__(self, model_name, n_classes=2, config=None):
        super(CodeT5FeatureExtractor, self).__init__()
        self.config = config or Config()
        
        # Load and freeze CodeT5 encoder
        self.codet5_encoder = T5EncoderModel.from_pretrained(model_name)
        for param in self.codet5_encoder.parameters():
            param.requires_grad = False
        
        # Set to evaluation mode to disable dropout in CodeT5
        self.codet5_encoder.eval()
        
        # Enhanced classifier head with multiple layers
        layers = []
        input_dim = self.codet5_encoder.config.d_model  # CodeT5 uses d_model instead of hidden_size
        ...
        self.attention_pooling = nn.MultiheadAttention(
            embed_dim=self.codet5_encoder.config.d_model,
            num_heads=8,
            dropout=0.1,
            batch_first=True
        )
    ...
    def forward(self, input_ids, attention_mask, return_features=False):
        # Extract features with frozen CodeT5
        with torch.no_grad():
            outputs = self.codet5_encoder(
                input_ids=input_ids,
...
class AssemblyTrainer:
    ...
    def train_epoch(self, dataloader, optimizer, scheduler):
        self.model.train()
        # Keep CodeT5 in eval mode
        self.model.codet5_encoder.eval()
    ...
    def train(self, train_loader, val_loader, test_loader, optimizer, scheduler, model_name):
        print(f"\n{'='*60}")
        print(f"Training Deep Learning Approach (Frozen CodeT5 + Neural Classifier)")
        print(f"{'='*60}")
...
def run_experiment(seed, config, evaluator):
    ...
    for model_name in config.MODEL_NAMES.values():
        ...
        model = CodeT5FeatureExtractor(model_name, config=config).to(config.get_device())
        model.codet5_encoder.resize_token_embeddings(len(tokenizer))
