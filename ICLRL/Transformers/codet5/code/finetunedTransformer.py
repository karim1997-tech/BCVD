...
class Config:
    ...
    SAVE_DIR = "E:/models/workB/ICLRL/Transformers/codet5/results/seed3"
    METRICS_FILE = "E:/models/workB/ICLRL/Transformers/codet5/results/seed3/training_metrics.csv"
...
class AssemblyClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        # For encoder-decoder models, we use the encoder outputs
        ...
        # Use the first token's hidden state as the pooled output
        pooled_output = encoder_outputs.last_hidden_state[:, 0, :]
        pooled_output = self.dropout(pooled_output)
        return self.classifier(pooled_output), None  # Return None for attentions to match your existing code
...
def train_model():
    ...
    for model_name in config.MODEL_NAMES.values():
        ...
        # Initialize model
        model = AssemblyClassifier(model_name).to(config.DEVICE)
